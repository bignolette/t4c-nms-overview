...
import { 
  Search, MapPin, Leaf, RotateCcw, Info, ExternalLink, Tag, Map as MapIcon, X, Filter, BarChart3
} from 'lucide-react';
import Pagination from './shared/Pagination';

interface HarvestListProps {
  resources: any[]; // Using any to accommodate the specific JSON structure
}
...
const ResourceCard = memo(({ resource }: { resource: any }) => {
  ...
  return (
    <motion.div 
      ...
      className={`group relative bg-slate-800/40 border ${isTree ? 'border-emerald-500/30' : 'border-green-500/30'} rounded-2xl overflow-hidden transition-all duration-300 shadow-xl hover:shadow-emerald-500/10 hover:border-emerald-500/50 flex flex-col`}
    >
      <div className="p-5 bg-slate-900/40 border-b border-slate-700/50 relative overflow-hidden">
        <div className="flex justify-between items-start mb-4 relative z-10">
          <div className="flex-1 min-w-0 pr-2">
            ...
            <h3 className="text-xl font-black group-hover:text-emerald-400 transition-colors leading-tight italic tracking-tight text-slate-100 flex items-center gap-2 overflow-hidden mt-2">
              <span>{resource.name}</span>
            </h3>
          </div>
          <div className={`p-2 rounded-lg ${isTree ? 'bg-emerald-500/10 text-emerald-500' : 'bg-green-500/10 text-green-500'} shrink-0`}>
            <Leaf size={18} />
          </div>
        </div>
      </div>
      ...
      <div className="p-5 flex-1 flex flex-col gap-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {coords.length > 6 && (
                <button 
                  ...
                  className="text-[10px] font-black text-slate-500 hover:text-emerald-500 uppercase tracking-widest px-2"
                >
        ...
        <div className="mt-auto pt-4 border-t border-slate-700/30">
          <Link 
            to={`/wiki/metiers?search=${encodeURIComponent(resource.name)}`}
            className="w-full flex items-center justify-center gap-2 py-2.5 bg-emerald-500/10 hover:bg-emerald-500 text-emerald-500 hover:text-slate-950 border border-emerald-500/20 rounded-xl transition-all font-black uppercase tracking-widest text-[10px]"
          >
...
const HarvestList = ({ resources }: HarvestListProps) => {
  const [searchParams, setSearchParams] = useSearchParams();
  const urlSearch = searchParams.get('search') || '';

  const [selectedZone, setSelectedZone] = useState<string>('Toutes');
  const [selectedType, setSelectedType] = useState<'Tous' | 'Plante' | 'Arbre'>('Tous');
  ...
  const allZones = useMemo(() => {
    ...
    resources.forEach(r => { 
      if (Array.isArray(r.zone)) r.zone.forEach((z: string) => zonesSet.add(z));
      else if (r.zone) zonesSet.add(r.zone);
    });
  ...
  const filteredResources = useMemo(() => {
    ...
    return resources.filter(r => {
      ...
      return matchesSearch && matchesType && matchesZone && matchesRarity;
    }).sort((a, b) => a.name.localeCompare(b.name));
  }, [resources, selectedZone, selectedType, selectedRarity, activeSearchTerm]);
    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-6 bg-slate-800/30 p-4 md:p-6 rounded-2xl border border-slate-700/50 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row gap-4 relative z-10">
          <div className="flex-1 relative group">
            ...
            <input 
              type="text"
              placeholder="Rechercher une plante ou un bois..."
              value={activeSearchTerm}
              onChange={(e) => handleSearchChange(e.target.value)}
              className="w-full bg-slate-950/50 border border-slate-800 rounded-xl py-3.5 pl-12 pr-12 text-slate-100 focus:border-emerald-500/50 outline-none transition-all font-bold"
            />
            {activeSearchTerm && (
          ...
          <div className="flex flex-wrap md:flex-nowrap gap-2 shrink-0">
            ...
            <div className="flex items-center gap-4 px-4 py-2 bg-slate-950/50 rounded-xl border border-slate-800 shadow-inner">
              <div className="text-slate-400 whitespace-nowrap flex items-center gap-2">
                <span className="font-black text-emerald-500 text-lg">{filteredResources.length}</span>
                <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest opacity-60">/ {resources.length}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 relative z-10">
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-slate-500 ml-1">
              ...
              <label htmlFor="harvest-type-select" className="text-[10px] font-black uppercase tracking-[0.2em]">Type</label>
            </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-slate-500 ml-1">
              ...
              <label htmlFor="harvest-rarity-select" className="text-[10px] font-black uppercase tracking-[0.2em]">Rareté</label>
            </div>
            <div className="relative">
              <select 
                id="harvest-rarity-select"
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-slate-500 ml-1">
              ...
              <label htmlFor="harvest-zone-select" className="text-[10px] font-black uppercase tracking-[0.2em]">Zone</label>
            </div>
            <div className="relative">
              <select 
                id="harvest-zone-select"
                value={selectedZone}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <AnimatePresence mode="popLayout">
          {paginatedResources.map((res, idx) => (
            <ResourceCard key={res.name + idx} resource={res} />
          ))}
        </AnimatePresence>
      </div>
      ...
      {filteredResources.length === 0 && (
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-16 text-center">
          ...
          <h3 className="text-xl font-bold text-slate-300">Aucune ressource trouvée</h3>
