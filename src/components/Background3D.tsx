...
// Color palettes per page context
const PALETTES: Record<string, { magic: [number,number,number][]; fire: [number,number,number][] }> = {
  default: {
    magic: [[0.1, 0.4, 1.0], [0.2, 0.9, 1.0], [0.6, 0.1, 1.0]],
    fire:  [[1.0, 0.6, 0.0], [1.0, 0.2, 0.0], [1.0, 0.4, 0.1]],
  },
  bestiary: {
    magic: [[0.8, 0.1, 0.2], [1.0, 0.2, 0.3], [0.6, 0.0, 0.1]],
    fire:  [[1.0, 0.1, 0.0], [0.8, 0.0, 0.0], [1.0, 0.3, 0.1]],
  },
  spells: {
    magic: [[0.3, 0.1, 1.0], [0.6, 0.2, 1.0], [0.1, 0.5, 1.0]],
    fire:  [[0.5, 0.0, 1.0], [0.8, 0.3, 1.0], [0.2, 0.1, 0.9]],
  },
  recipes: {
    magic: [[1.0, 0.6, 0.1], [1.0, 0.8, 0.2], [0.9, 0.4, 0.0]],
    fire:  [[1.0, 0.5, 0.0], [1.0, 0.7, 0.1], [0.8, 0.3, 0.0]],
  },
  maps: {
  ...
  planner: {
    magic: [[0.2, 0.9, 1.0], [0.1, 0.7, 0.9], [0.3, 1.0, 0.8]],
    fire:  [[0.0, 0.8, 1.0], [0.1, 0.6, 0.8], [0.2, 1.0, 0.9]],
  },
  quests: {
    magic: [[0.9, 0.7, 0.1], [1.0, 0.85, 0.3], [0.8, 0.5, 0.0]],
    fire:  [[1.0, 0.8, 0.2], [0.9, 0.6, 0.0], [1.0, 0.9, 0.4]],
  },
};

function getPalette(pathname: string) {
  if (pathname.includes('bestiary')) return PALETTES.bestiary;
  if (pathname.includes('spells') || pathname.includes('skills')) return PALETTES.spells;
  if (pathname.includes('recipes') || pathname.includes('metiers')) return PALETTES.recipes;
  if (pathname.includes('maps')) return PALETTES.maps;
  if (pathname.includes('planner')) return PALETTES.planner;
  if (pathname.includes('quests')) return PALETTES.quests;
