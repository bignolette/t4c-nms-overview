      ...
      >
        ...
        <div className="relative flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="space-y-4">
            ...
            {/* Instant Search Bar */}
            <div className="relative max-w-xl search-input-glow rounded-2xl">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 transition-all duration-300 search-icon" size={20} />
              <input
                type="text"
                placeholder="Rechercher une quête, zone, boss..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full pl-12 pr-12 py-4 bg-slate-950/50 border border-slate-800 rounded-2xl text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-amber-500/40 transition-all shadow-inner font-bold"
              />
              {searchTerm && (
                <button
                  onClick={() => setSearchTerm('')}
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 hover:text-white transition-all hover:rotate-90 duration-200"
                >
                  <X size={18} />
                </button>
              )}
    ...
    <div className="pb-12 space-y-6 relative">
      ...
      {currentStepIdx === totalSteps - 1 && quest.boss_fight && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="relative overflow-hidden rounded-2xl border border-red-500/30 bg-gradient-to-br from-slate-900/80 via-red-950/10 to-slate-900/80 backdrop-blur-md p-8 shadow-[0_0_30px_rgba(239,68,68,0.15)] hover:shadow-[0_0_40px_rgba(239,68,68,0.25)] transition-shadow duration-500"
        >
          <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-[0.03]" />
          <div className="relative">
            <div className="flex items-center gap-3 mb-6">
              <div className="p-3 bg-red-500/10 rounded-xl border border-red-500/20">
                <Skull size={28} className="text-red-400 animate-pulse" />
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                ...
                <div className="p-4 bg-red-500/5 rounded-xl border border-red-500/20">
                  <p className="text-[10px] text-red-400 uppercase tracking-wider font-bold mb-2 flex items-center gap-1.5"><AlertTriangle size={10} className="animate-pulse" /> Capacités</p>
                  <ul className="space-y-1.5">
